...
def extract_text_multi(categorized_corpus, args, norm_words=None):
	...
	train_instances = ((norm_words(words), cats) for words, cats in train_instances)
	train_instances = [(words, cats) for words, cats in train_instances if words]
	
	test_instances = lif(categorized_corpus, args.test_prefix)
	test_instances = ((norm_words(words), cats) for words, cats in test_instances)
	test_instances = [(words, cats) for words, cats in train_instances if words]
...
def cat_words_from_instances(instances):
	return ((cat, (word for i in instance_list for word in i)) for cat, instance_list in instances.iteritems())

def cat_words_from_instances_multi(multi_instances):
	cat_words = defaultdict([])
	for (words, cats) in multi_instances:
...
def get_featx(train_instances, args, cat_words=cat_words_from_instances):
	##################
	## word scoring ##
	##################
	...
	if args.min_score or args.max_feats:
		...
		cat_words = cat_words(train_instances)
...
def train(train_feats, labels, args):
	##############
	## training ##
	##############
...
def evaluate(classifier, test_feats, labels, args):
	################
	## evaluation ##
	################
	stats = {}
	if not args.no_accuracy:
		try:
			acc = accuracy(classifier, test_feats)
		except ZeroDivisionError:
			acc = 0
		stats['accuracy'] = acc

	if args.multi and args.binary and not args.no_masi_distance:
		stats['average masi distance'] = scoring.avg_masi_distance(classifier, test_feats)
	...
	if not args.no_precision or not args.no_recall or not args.no_fmeasure:
		...
		for label in labels:
			...
			if not args.no_precision:
				stats[" ".join((label, "precision"))] = precision(ref, test) or 0
			
			if not args.no_recall:
				stats[" ".join((label, "recall"))] = recall(ref, test) or 0
			
			if not args.no_fmeasure:
				stats[" ".join((label, "f-measure"))] = f_measure(ref, test) or 0
	return stats


def pickle(classifier, args):
	##############
	## pickling ##
	##############
	if args.filename:
...
if __name__ == '__main__':
	...
	categorized_corpus = get_corpus(args)
	
	# get labels and validate
	...
	if args.cross_fold:
		...
		classifier = train(train_feats, labels, args)
	
		if not args.no_eval:
			stats = evaluate(classifier, test_feats, labels, args)
			stat_keys = ['accuracy','average_masi_distance']
			for label in labels:
				stat_keys.extend([" ".join([label, stat]) for stat in ("precision", "recall", "f-measure")])
			for mkey in stat_keys:
				if stats.has_key(mkey):
					print '%s: %f' % (mkey, stats[mkey])
